...
var generateHeightmap = require("./heightmap");
bitmap.onload = function(e) {
  var map = generateHeightmap(e.target, HEIGHTMAP_DENSITY, HEIGHTMAP_SIZE, HEIGHTMAP_SCALE);
...
var points = [];
console.log(MAP_BOUNDS[0], MAP_BOUNDS[1]);
console.log(MAP_EXTENT);
console.log(MAP_CENTER);
window.depredationData.forEach(function(p) {
  var dx = (MAP_CENTER[1] - p.lng) / MAP_EXTENT[1];
  var dy = (MAP_CENTER[0] - p.lat) / MAP_EXTENT[0];
  var px = MAP_CENTER[1] + dx * HEIGHTMAP_SIZE;
  var py = MAP_CENTER[0] + dy * HEIGHTMAP_SIZE;
  points.push(px, 1, py);
});
...
var drawPoints = function(points) {
  points.push(landscape.position.x, 1, landscape.position.z);
